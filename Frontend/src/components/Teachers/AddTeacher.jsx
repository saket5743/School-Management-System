import React, { useState } from 'react';
import Sidebar from '../Sidebar/Sidebar';
import { FiBell } from 'react-icons/fi';
import { useNavigate } from 'react-router-dom';

const AddTeacher = () => {
  const navigate = useNavigate();
  const [inputValue, setInputValue] = useState({
    fullname: '',
    email: '',
    designation: '',
    classAssigned: '',
    gender: '',
    password: '',
    phone_number: '',
    subject: '',
  });
  const [teachers, setTeachers] = useState([]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    setInputValue((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  // const handleAddTeacher = () => {
  //   const newTeacher = {
  //     id: Date.now(),
  //     ...inputValue
  //   };

  //   setTeachers((prevTeachers) => [...prevTeachers, newTeacher]);

  //   const existingTeachers = JSON.parse(localStorage.getItem('teachers')) || [];
  //   const updatedTeachers = [...existingTeachers, newTeacher];
  //   localStorage.setItem('teachers', JSON.stringify(updatedTeachers));

  //   setInputValue({
  //     fullName: '',
  //     email: '',
  //     designation: '',
  //     classAssigned: '',
  //     gender: '',
  //     password: '',
  //     phone: '',
  //     subject: '',
  //   });

  //   navigate('/listteachers');
  // };

  const handleAddTeacher = async () => {
    if (![
      inputValue.fullname &&
      inputValue.email &&
      inputValue.classAssigned &&
      inputValue.gender &&
      inputValue.password &&
      inputValue.phone_number &&
      inputValue.subject
    ]) {
      alert("All fields are required.");
      return;
    }
  
    try {
      const response = await fetch(
        "http://localhost:3151/api/v1/teacher/registerteacher",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(inputValue),
        }
      );
  
      if (!response.ok) {
        const errorData = await response.json();
        console.error("Server Error:", errorData);
        alert(`Error: ${errorData.message || 'Internal Server Error'}`);
        return;
      }
  
      const result = await response.json();
      console.log(result, "resultsssss");
      navigate("/listteachers");
    } catch (error) {
      console.error("Fetch Error:", error);
      alert("An unexpected error occurred.");
    }
  };
  
  const isFormValid = () => {
    return (
      inputValue.fullname.trim() !== '' &&
      inputValue.email.trim() !== '' &&
      // inputValue.designation.trim() !== '' &&
      inputValue.classAssigned.trim() !== '' &&
      inputValue.gender.trim() !== '' &&
      inputValue.password.trim() !== '' &&
      inputValue.phone_number.trim() !== '' &&
      inputValue.subject.trim() !== ''
    );
  };

  return (
    <div className="flex">
      <div>
        <Sidebar />
      </div>
      <div>
        <div className="flex justify-end w-[184vh] p-2 gap-4">
          <div className="w-[24px] h-[24px] p-2">
            <FiBell className="text-xl" />
          </div>
          <div className="w-[120px] h-[40px] p-1">
            <button onClick={() => navigate("/dashboard")}>Log out</button>
          </div>
        </div>

        <div className="w-full max-w-5xl bg-white p-8 ml-[130px] mt-[30px] rounded-lg shadow-md">
          <div className="flex justify-between items-start mb-8">
            <h2 className="text-3xl font-semibold text-gray-800">
              Add Teachers
            </h2>
            <div className="w-64">
              <label className="block text-sm text-gray-600 mb-1">
                Designation
              </label>
              <input
                type="text"
                className="w-full border border-gray-300 rounded px-3 py-2 outline-none"
                placeholder="Enter designation"
                name="designation"
                value={inputValue.designation}
                onChange={handleChange}
              />
            </div>
          </div>

          <div className="flex gap-12 pb-5">
            <button className="text-gray-900 border-black pb-1">
              Manually
            </button>
            <button className="text-gray-900 border-black pb-1">
              Import CSV
            </button>
          </div>

          <div className="space-y-3">
            <div>
              <label className="block text-sm text-gray-600 mb-1">
                Full Name
              </label>
              <input
                type="text"
                className="w-full border border-gray-300 rounded px-3 py-2 outline-none"
                placeholder="Enter full name"
                name="fullname"
                value={inputValue.fullname}
                onChange={handleChange}
              />
            </div>

            <div className="flex gap-[110px]">
              <div>
                <label className="block text-sm text-gray-600 mb-1">
                  Email address
                </label>
                <input
                  type="email"
                  className="w-[380px] border border-gray-300 rounded px-3 py-2 outline-none"
                  placeholder="example@email.com"
                  name="email"
                  value={inputValue.email}
                  onChange={handleChange}
                />
              </div>
              <div className="flex gap-7">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
                    Class
                  </label>
                  <select
                    name="classAssigned"
                    value={inputValue.classAssigned}
                    onChange={handleChange}
                    className="w-[190px] text-gray-600 border border-gray-300 rounded px-3 py-2 outline-none"
                  >
                    <option value="">Select Class</option>
                    <option value="1">1</option>
                    <option value="2">2</option>
                    <option value="3">3</option>
                    <option value="4">4</option>
                    <option value="5">5</option>
                    <option value="6">6</option>
                    <option value="7">7</option>
                    <option value="8">8</option>
                    <option value="9">9</option>
                    <option value="10">10</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">
                    Gender
                  </label>
                  <select
                    name="gender"
                    value={inputValue.gender}
                    onChange={handleChange}
                    className="w-[190px] border text-gray-600 border-gray-300 rounded px-3 py-2 outline-none"
                  >
                    <option value="">Select Gender</option>
                    <option value="Male">Male</option>
                    <option value="Female">Female</option>
                    <option value="others">Others</option>
                  </select>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-600 mb-1">
                  Password
                </label>
                <input
                  type="password"
                  className="w-[380px] border border-gray-300 rounded px-3 py-2 outline-none"
                  placeholder="Enter password"
                  name="password"
                  value={inputValue.password}
                  onChange={handleChange}
                />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">
                  Phone number
                </label>
                <input
                  type="tel"
                  className="w-full border border-gray-300 rounded px-3 py-2 outline-none"
                  placeholder="Enter phone number"
                  name="phone_number"
                  value={inputValue.phone_number}
                  onChange={handleChange}
                />
              </div>
            </div>

            <div>
              <label className="block text-sm text-gray-600 mb-1">
                Subject
              </label>
              <select
                name="subject"
                value={inputValue.subject}
                onChange={handleChange}
                className="w-full border text-gray-600 border-gray-300 rounded px-3 py-2 outline-none"
              >
                <option value="">Select Subject</option>
                <option value="Sanskrit">Sanskrit</option>
                <option value="Hindi">Hindi</option>
                <option value="English">English</option>
                <option value="Maths">Maths</option>
                <option value="Science">Science</option>
                <option value="Social Science">Social Science</option>
                <option value="Computer">Computer</option>
              </select>
            </div>
          </div>

          <div className="flex justify-start mt-10 gap-12">
            <button className="flex items-center text-gray-600 hover:text-gray-800">
              <span className="text-xl mr-1">＋</span> Add another
            </button>
            <button
              disabled={!isFormValid()}
              onClick={handleAddTeacher}
              className="bg-gray-300 text-black px-6 py-2 rounded hover:bg-gray-800 hover:text-white transition"
            >
              Add Teacher
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AddTeacher;
